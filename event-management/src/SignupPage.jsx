...
function getPasswordStrength(pw) {
  ...
  if (score <= 1) return { label: "Weak",   color: "bg-[#ef4444]", text: "text-[#ef4444]",   bars: 1 };
  if (score <= 2) return { label: "Fair",   color: "bg-[#fb923c]", text: "text-[#fb923c]",   bars: 2 };
  if (score <= 3) return { label: "Good",   color: "bg-[#fbbf24]", text: "text-[#fbbf24]",   bars: 3 };
  if (score <= 4) return { label: "Strong", color: "bg-[#a3e635]", text: "text-[#a3e635]",   bars: 4 };
  return            { label: "Very Strong", color: "bg-[#a3e635]", text: "text-[#a3e635]",   bars: 5 };
}
...
function Field({ label, required, error, children }) {
  return (
    <div>
      <label className="flex items-center gap-1 text-xs font-semibold text-[#5a5a62] uppercase tracking-wider mb-2">
        {label}
...
export default function SignUpPage() {
  ...
  const handleSubmit = async () => {
    ...
    navigate(`/verify-otp?email=${encodeURIComponent(form.email)}&role=${role}&mode=signup`);
  };
  ...
  const inputClass = (field) =>
    `w-full bg-[#0c0c0f] border rounded-xl px-4 py-3 text-sm text-white outline-none transition-colors placeholder:text-[#3a3a42] ${
      errors[field] ? "border-[#ef4444]" : "border-[#1e1e22] focus:border-[#a3e635]"
    }`;
  ...
  return (
    <div className="min-h-screen bg-[#0c0c0f] text-white flex flex-col">
      ...
      {/* ── HEADER ──────────────────────────────────────────────────── */}
      <header className="flex items-center justify-between px-12 h-16 border-b border-[#1e1e22] sticky top-0 z-50 bg-[#0c0c0f]/90 backdrop-blur-md">
        ...
        <p className="text-sm text-[#5a5a62]">
          ...
          <Link to="/login" className="text-[#a3e635] font-semibold hover:underline">Log In</Link>
        </p>
      </header>
      ...
      <main className="flex-1 flex items-start justify-center px-6 py-12">
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-8">
            <p className="text-xs font-bold uppercase tracking-widest text-[#a3e635] mb-3">Step 2 of 2</p>
            ...
            <p className="text-[#5a5a62] text-sm">
              ...
              {" · "}
              <Link to="/roleselect" className="text-[#a3e635] hover:underline text-xs">Change role</Link>
            </p>
          </div>
          ...
          <div className="bg-[#111115] border border-[#1e1e22] rounded-2xl p-8">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-xs text-[#3a3a42] font-semibold uppercase tracking-wider">or fill in details</span>
              <div className="flex-1 h-px bg-[#1e1e22]" />
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Username" required error={errors.username}>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#3a3a42] text-sm">@</span>
                    <input type="text" placeholder="alexj"
              ...
              <Field
                ...
                error={errors.phone}
              >
                <input type="tel" placeholder={isOrganiser ? "+91 98765 43210 (required)" : "+91 98765 43210 (optional)"}
                  value={form.phone} onChange={set("phone")}
                  className={inputClass("phone")} />
                {isOrganiser && !errors.phone && (
                  <p className="text-xs text-[#818cf8] mt-1">Required for organisers — attendees may contact you.</p>
                )}
              </Field>
              ...
              <Field label="Password" required error={errors.password}>
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPw(!showPw)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5a5a62] hover:text-white transition-colors text-xs font-bold">
                    {showPw ? "HIDE" : "SHOW"}
                  </button>
                </div>
                ...
                {!errors.password && (
                  <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-0.5">
                    {[
                      ...
                      <p key={label} className={`text-xs flex items-center gap-1 ${rule ? "text-[#a3e635]" : "text-[#3a3a42]"}`}>
              ...
              <Field label="Confirm Password" required error={errors.confirm}>
                ...
                {form.confirm && form.password && !errors.confirm && (
                  <p className={`text-xs mt-1 flex items-center gap-1 ${form.password === form.confirm ? "text-[#a3e635]" : "text-[#ef4444]"}`}>
