...
import { CreditCard, Building2, Smartphone, ChevronLeft, Lock, Check, X, AlertCircle, Loader } from "lucide-react";
...
const STEPS = ["Initiating payment", "Verifying details", "Processing transaction", "Finalising"];
...
const PaymentPage = () => {
  ...
  const { event, selectedProgrammes = [], totalPrice = 0 } = location.state || {};
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${API}/api/bank/account?userId=${CURRENT_USER_ID}`);
        if (!res.ok) throw new Error();
        setUserAccount(await res.json());
      } catch {
        // Fallback to mock H2 data
        setUserAccount(MOCK_BANK_ACCOUNTS.find(a => a.userId === CURRENT_USER_ID) || MOCK_BANK_ACCOUNTS[0]);
      }
    })();
  }, []);
  ...
  const validatePayment = () => {
    ...
    if (method === "card") {
      ...
      if (cleanInput !== cleanStored)  return { ok: false, reason: "Card number does not match our records." };
      if (cvv !== userAccount.cvv)     return { ok: false, reason: "Incorrect CVV." };
      if (expiry !== userAccount.expiry) return { ok: false, reason: "Card expiry does not match." };
      if (userAccount.balance < totalPrice) return { ok: false, reason: `Insufficient balance. Available: ₹${userAccount.balance.toFixed(2)}` };
      return { ok: true };
    }

    if (method === "netbanking") {
      if (nbUserId !== userAccount.netBankingId)   return { ok: false, reason: "Incorrect net banking user ID." };
      if (nbPassword !== userAccount.netBankingPw) return { ok: false, reason: "Incorrect net banking password." };
      if (userAccount.balance < totalPrice)        return { ok: false, reason: `Insufficient balance. Available: ₹${userAccount.balance.toFixed(2)}` };
      return { ok: true };
    }

    if (method === "upi") {
      if (!upiId.includes("@")) return { ok: false, reason: "Please enter a valid UPI ID (e.g. name@bank)." };
      // UPI: random 80% success / 20% failure
      const roll = Math.random();
      if (roll < 0.20) return { ok: false, reason: "UPI transaction failed — network timeout. Please try again." };
      if (userAccount.balance < totalPrice) return { ok: false, reason: `Insufficient balance. Available: ₹${userAccount.balance.toFixed(2)}` };
      return { ok: true };
    }
    ...
    if (ok) {
      ...
      try {
        await fetch(`${API}/api/bank/deduct`, {
          ...
          body: JSON.stringify({ userId: CURRENT_USER_ID, amount: totalPrice, eventId: event.id }),
        });
      } catch { /* silent — mock data handles it */ }
  ...
  const canPay = () => {
    if (method === "card")       return cardNumber.replace(/\s/g,"").length === 16 && cvv.length >= 3 && expiry.length === 5 && cardName.trim().length > 0;
    if (method === "netbanking") return nbUserId.trim().length > 0 && nbPassword.trim().length > 0;
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9] font-sans selection:bg-lime-200">
      ...
      <nav className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
        ...
        {!processing && (
          <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm font-medium text-gray-600 hover:text-black transition">
          ...
          <div className="lg:col-span-3">
            ...
            {processing && (
              <div className="bg-white border border-gray-100 p-10 text-center mb-6">
                ...
                <h3 className="text-xl font-bold text-black mb-6">{STEPS[step]}...</h3>
                <div className="flex justify-center gap-2">
                  {STEPS.map((s, i) => (
                    <div key={i} className={`h-1.5 rounded-full transition-all duration-500 ${
                      i < step ? "bg-[#C4F249] w-8" : i === step ? "bg-black w-8 animate-pulse" : "bg-gray-200 w-4"
                    }`} />
                  ))}
                </div>
            ...
            {result === "success" && !processing && (
              <div className="bg-white border-2 border-[#C4F249] p-10 text-center mb-6">
                ...
                <p className="text-gray-500 text-sm">₹{totalPrice} debited. Redirecting to your ticket...</p>
              </div>
            )}
            ...
            {result === "failure" && !processing && (
              <div className="bg-white border-2 border-red-400 p-8 mb-6">
                ...
                {method === "upi" && (
                  <p className="text-xs text-gray-400 text-center mt-3">UPI transactions may fail due to network issues. Please retry.</p>
                )}
              </div>
            )}
              ...
              <>
                  ...
                  {methods.map(m => (
                    <button key={m.key} onClick={() => setMethod(m.key)}
                      className={`flex items-center gap-2 px-5 py-3 text-sm font-bold transition border-b-2 -mb-px ${
                        method === m.key ? "border-black text-black" : "border-transparent text-gray-400 hover:text-gray-600"
                      }`}>
                      {m.icon} {m.label}
                    </button>
                  ))}
                ...
                {method === "card" && (
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Card Number</label>
                      <input
                        type="text"
                        value={cardNumber}
                    ...
                    <div>
                      <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Name on Card</label>
                      <input
                        type="text"
                        value={cardName}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Expiry</label>
                        <input
                          type="text"
                          value={expiry}
                      ...
                      <div>
                        <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">CVV</label>
                        <input
                          type="password"
                          value={cvv}
                    ...
                    <div className="bg-blue-50 border border-blue-100 p-3 text-xs text-blue-600 flex gap-2">
                      ...
                      <span>Test card: <strong className="font-mono">4111 1111 1111 1111</strong> · Expiry: <strong>12/28</strong> · CVV: <strong>737</strong></span>
                    </div>
                  </div>
                )}
                ...
                {method === "netbanking" && (
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Select Bank</label>
                      <div className="grid grid-cols-3 gap-2 mb-4">
                        {[
                          ...
                          <button key={b.key} onClick={() => setBankName(b.key)}
                    ...
                    <div>
                      <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Net Banking User ID</label>
                      <input type="text" value={nbUserId} onChange={e => setNbUserId(e.target.value)}
                    ...
                    <div>
                      <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Password</label>
                      <input type="password" value={nbPassword} onChange={e => setNbPassword(e.target.value)}
                        placeholder="Your net banking password"
                        className="w-full border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:border-black transition" />
                    </div>
                    <div className="bg-blue-50 border border-blue-100 p-3 text-xs text-blue-600 flex gap-2">
                      <AlertCircle size={13} className="shrink-0 mt-0.5" />
                      <span>Test credentials: User ID: <strong>dutta.aniruddha</strong> · Password: <strong>Bank@1234</strong></span>
                    </div>
                  </div>
                )}
                ...
                {method === "upi" && (
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">UPI ID</label>
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 p-3 text-xs text-yellow-700 flex gap-2">
                      <AlertCircle size={13} className="shrink-0 mt-0.5" />
                      <span>UPI transactions have an 80% success rate. Failures simulate real network timeouts. Test ID: <strong>ishaaaaan@okaxis</strong></span>
                    </div>
            ...
            <div className="bg-white border border-gray-100 p-6">
              ...
              <div className="space-y-2 mb-4 border-t border-gray-100 pt-4">
                {selectedProgrammes.map(p => (
                  <div key={p.id} className="flex justify-between items-start text-sm">
                    ...
                    <span className="font-bold text-black ml-3 shrink-0">₹{p.price}</span>
                  </div>
                ))}
              </div>
              ...
              {userAccount && (
                <div className="mt-4 bg-gray-50 p-3 text-xs text-gray-500">
                  <p className="font-semibold text-gray-700 mb-0.5">Linked Account</p>
                  <p>{userAccount.bankName} · ····{userAccount.accountNo.slice(-4)}</p>
                  <p>Available: <span className="font-bold text-black">₹{userAccount.balance.toFixed(2)}</span></p>
                </div>
              )}
            </div>
