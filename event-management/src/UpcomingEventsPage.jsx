  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setEvents(MOCK_EVENTS.filter(e => e.status === "upcoming" && new Date(e.startDate) > today));
      }
    })();
  }, []);
    ...
    <div className="min-h-screen bg-[#F9F9F9] font-sans selection:bg-lime-200">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8 pb-10">
        ...
        <p className="text-gray-400 text-sm mt-3">
          {events.length} event{events.length !== 1 ? "s" : ""} coming up — book your spot early
      ...
      <div className="max-w-7xl mx-auto px-6 mb-8 flex gap-3">
        {[["all", "All Events"], ["free", "Free Only"], ["paid", "Paid Only"]].map(([val, label]) => (
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20">
        {pageEvents.length === 0 ? (
          <div className="bg-white border border-dashed border-gray-200 py-24 text-center">
            <p className="text-gray-400">No upcoming events matching your filter</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-10">
            {pageEvents.map(event => {
              const totalSeats = event.programmes?.reduce((s, p) => s + (p.seatsLeft || 0), 0) || 0;
              const minPrice   = event.type === "Paid"
                ...
                <div key={event.id}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-start justify-between gap-2 mb-3">
                      ...
                      <span className={`text-xs font-bold px-2.5 py-1 shrink-0 ${event.type === "Paid" ? "bg-black text-[#C4F249]" : "bg-[#C4F249] text-black"}`}>{event.type}</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 text-xs text-gray-400 mb-4">
                      ...
                      <span className="flex items-center gap-1"><Users size={12} />{totalSeats.toLocaleString()} seats left</span>
                    </div>
                    ...
                    <div className="mb-4 space-y-1">
                      {(event.programmes || []).slice(0, 2).map(p => (
                        <div key={p.id} className="flex justify-between items-center text-xs py-1 border-b border-gray-50">
                          ...
                          <span className="font-bold text-black ml-2 shrink-0">{event.type === "Paid" ? `₹${p.price}` : "Free"}</span>
                        </div>
                      ))}
                      {(event.programmes?.length || 0) > 2 && (
                        <p className="text-xs text-gray-400">+ {event.programmes.length - 2} more programme{event.programmes.length - 2 > 1 ? "s" : ""}</p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      {event.type === "Paid" && (
                        <div>
                          ...
                          <p className="text-xl font-bold text-black">₹{minPrice}</p>
                        </div>
                      )}
                      <button
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2">
            <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1}
              className="px-4 py-2 border border-gray-200 text-sm font-medium disabled:opacity-30 hover:border-black transition">← Prev</button>
            {Array.from({length: totalPages},(_,i)=>i+1).map(p=>(
              <button key={p} onClick={()=>setPage(p)}
                className={`w-9 h-9 text-sm font-bold transition ${page===p?"bg-black text-[#C4F249]":"border border-gray-200 text-gray-600 hover:border-black"}`}>{p}</button>
            ))}
            <button onClick={() => setPage(p => Math.min(totalPages,p+1))} disabled={page===totalPages}
              className="px-4 py-2 border border-gray-200 text-sm font-medium disabled:opacity-30 hover:border-black transition">Next →</button>
          </div>
        )}
      </div>
    </div>
