...
import { Calendar, Clock, Tag, ArrowLeft, Star, MessageSquare, Users } from "lucide-react";
        ...
        return (
            <div className="flex items-center gap-1.5">
                <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map(s => (
                        <Star key={s} size={12}
                            className={parseFloat(rating) >= s ? "fill-[#C4F249] text-[#C4F249]" : "text-gray-200"} />
                    ))}
                </div>
        ...
        <div className="min-h-screen bg-[#F9F9F9] font-sans selection:bg-lime-200">
            ...
            <div className="max-w-7xl mx-auto px-6 pt-8 pb-10">
                <h1 className="text-6xl md:text-7xl font-normal leading-[1.1] tracking-tight mb-2">
                    <span className="text-gray-400">Completed</span><br />
                    <span className="text-black">Events</span>
                </h1>
                <p className="text-gray-400 text-sm mt-3">
                    {events.length} past event{events.length !== 1 ? "s" : ""} — browse and leave your feedback
                    {!backendOnline && <span className="ml-3 text-yellow-600 text-xs">⚠ Mock data</span>}
                </p>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-6 mb-8">
                <div className="bg-gray-100 border border-gray-200 px-5 py-3 flex items-center gap-3 text-sm text-gray-600">
                    ...
                    These events have ended. You can view details and leave a star rating + feedback for any event you attended.
                </div>
            </div>
                ...
                {pageEvents.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-10">
                        {pageEvents.map(event => {
                            ...
                            const totalProg = event.programmes?.reduce((s, p) => s + 1, 0) || 0;
                                    ...
                                    <div className="p-6 flex flex-col flex-1">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 mb-4">
                                            <div className="bg-gray-50 p-2 text-center">
                                                ...
                                                <p className="font-bold text-black text-sm">{progCount}</p>
                                            </div>
                                            <div className="bg-gray-50 p-2 text-center">
                                        ...
                                        <div className="pt-3 border-t border-gray-100 flex justify-between items-center">
                                            ...
                                            <span className="text-xs font-bold text-gray-500 group-hover:text-black transition">
                                                {fbCount > 0 ? `${fbCount} review${fbCount > 1 ? "s" : ""}` : "Be first to review"} →
                                            </span>
                                        </div>
                                    </div>
                                </div>
