...
const EmptyState = ({ icon: Icon, message, sub }) => (
  ...
  >
    ...
    {sub && (
      <p className="text-xs text-center max-w-xs leading-relaxed">{sub}</p>
    )}
  </div>
);
      ...
      >
        <table className="w-full text-sm">
          <thead>
            ...
            >
              {["ID", "Name", "Email", "Phone", "Username", "Actions"].map(
                (h, i) => (
                  <th
                    key={h}
          ...
          <tbody>
            {filtered.map((user, idx) => (
              <tr
                ...
                style={{
                  borderBottom:
                    idx < filtered.length - 1 ? "1px solid #f3f3f3" : "none",
                  background: "#fff",
                }}
...
const VenuesTab = ({ backendOnline }) => {
  ...
  const saveEdit = () => {
    ...
    setVenues((prev) =>
      prev.map((v) =>
        v.id === editingId
          ? { ...editForm, capacity: Number(editForm.capacity) }
          : v,
      ),
    );
    setEditingId(null);
      ...
      >
        <table className="w-full text-sm">
          <thead>
            ...
            >
              {["ID", "Venue Name", "Capacity", "Availability", "Actions"].map(
                (h, i) => (
                  <th
                    key={h}
          ...
          <tbody>
            {venues.map((venue, idx) => (
              <tr
                ...
                style={{
                  borderBottom:
                    idx < venues.length - 1 ? "1px solid #f3f3f3" : "none",
                  background: "#fff",
                }}
                className="transition hover:brightness-95"
              >
                  ...
                  <>
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{
                          background:
                            venue.availability === "Y" ? "#d1fae5" : "#fee2e2",
                          color:
                            venue.availability === "Y" ? "#065f46" : "#991b1b",
                        }}
                      >
                        {venue.availability === "Y"
                          ? "Available"
                          : "Unavailable"}
                      </span>
                    </td>
            ...
            <div className="space-y-4">
              {[
                {
                  label: "Venue Name",
                  key: "name",
                  placeholder: "e.g. Campus-7 Seminar Hall",
                  type: "text",
                },
                {
                  label: "Capacity",
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm((p) => ({ ...p, availability: e.target.value }))
                  }
                  className="w-full rounded-xl px-4 py-2.5 text-sm outline-none"
                  style={{ border: "1px solid #e5e5e5", color: "#1a1a1a" }}
                >
  ...
  const handleAction = (id, action) => {
    if (action === "delete") {
      ...
      setEvents((prev) =>
        prev.map((e) =>
          e.id === id
            ? { ...e, status: action === "approve" ? "upcoming" : "rejected" }
            : e,
        ),
      );
    }
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {event.status === "pending" && (
                    <>
                      <button
                        onClick={() =>
                          setConfirmAction({ id: event.id, action: "approve" })
                        }
                      ...
                      <button
                        onClick={() =>
                          setConfirmAction({ id: event.id, action: "reject" })
                        }
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition hover:opacity-80"
                  ...
                  <button
                    onClick={() =>
                      setConfirmAction({ id: event.id, action: "delete" })
                    }
                    className="p-1.5 rounded-lg transition hover:opacity-70"
                    style={{ color: "#9ca3af" }}
                  >
        ...
        <ConfirmDialog
          message={
            confirmAction.action === "delete"
              ? "This will permanently delete the event and all linked programmes."
              : confirmAction.action === "approve"
                ? "This will approve the event and make it visible to all users."
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {prog.status === "pending" && (
                    <>
                      <button
                        onClick={() =>
                          setConfirmAction({ id: prog.id, action: "approve" })
                        }
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition hover:opacity-80"
                        style={{ background: "#d1fae5", color: "#065f46" }}
                      >
                      ...
                      <button
                        onClick={() =>
                          setConfirmAction({ id: prog.id, action: "reject" })
                        }
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition hover:opacity-80"
                        style={{ background: "#fee2e2", color: "#991b1b" }}
                      >
                  ...
                  <button
                    onClick={() =>
                      setConfirmAction({ id: prog.id, action: "delete" })
                    }
                    className="p-1.5 rounded-lg transition hover:opacity-70"
                    style={{ color: "#9ca3af" }}
                  >
        ...
        <ConfirmDialog
          message={
            confirmAction.action === "delete"
              ...
              : confirmAction.action === "approve"
                ? "This will approve the programme and make it visible to users."
          ...
          <tbody>
            {organisers.map((org, idx) => (
              <tr
                ...
                style={{
                  borderBottom:
                    idx < organisers.length - 1 ? "1px solid #f3f3f3" : "none",
                  background: "#fff",
                }}
                className="transition hover:brightness-95"
              >
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: "#9ca3af" }}>
                  Review and approve or reject programme submissions.
                </p>
              </div>
              <button
                ...
                {pendingProgs.map((prog) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2 shrink-0">
                      <button
                        onClick={() => handleProgAction(prog.id)}
        ...
        >
          ...
          >
            ...
            <p className="text-xs mb-6" style={{ color: "#9ca3af" }}>
              Password will be SHA-256 hashed by the backend before storing.
            </p>
            <div className="space-y-4">
              {[
                {
                  ...
                  key: "name",
                  placeholder: "e.g. KIIT-DU",
                  type: "text",
                },
                {
                  label: "Email",
                  key: "email",
                  placeholder: "e.g. org@kiit.in",
                  type: "email",
                },
                {
                  label: "Phone",
...
export default function AdminPage() {
  ...
  const filteredPool = recipientPool.filter(
    (r) =>
  ...
  const addNewAdmin = () => {
    ...
    setAdmins((prev) => [
      ...prev,
      { id: newId, isProtected: false, ...newAdminForm },
    ]);
    setNewAdminForm({
      firstName: "",
      lastName: "",
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: "#F0F0F0" }}>
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        {/* ── Header bar ─────────────────────────────────────────────────────── */}
        <div
          className="flex items-center justify-between rounded-2xl px-6 py-4 shadow-sm"
          style={{ background: "#fff" }}
        >
        ...
        <div className="rounded-2xl shadow-sm" style={{ background: "#fff" }}>
          ...
          <div className="p-10">
            ...
            {!backendOnline && (
              ...
              >
                ...
                Backend not connected — admin profile cannot be loaded. Start
                your server at {API}.
              </div>
            )}
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-16">
              ...
              <div className="space-y-3 text-xl" style={{ color: "#1a1a1a" }}>
                <p>
                  ...
                  <span
                    style={{ color: backendOnline ? "#1a1a1a" : "#9ca3af" }}
                  >
                    {admin?.name ||
                      (backendOnline ? "—" : "Loads from backend")}
                  </span>
                </p>
                <p>
          ...
          {activeTab && (
            <div style={{ borderTop: "1px solid #f0f0f0" }} className="p-6">
              ...
              {activeTab === "Users" && (
                <UsersTab backendOnline={backendOnline} />
              )}
              {activeTab === "Venues" && (
                <VenuesTab backendOnline={backendOnline} />
              )}
              {activeTab === "Events" && (
                <EventsTab backendOnline={backendOnline} />
              )}
              {activeTab === "Programmes" && (
                <ProgrammesTab backendOnline={backendOnline} />
              )}
              {activeTab === "Organisers" && (
                <OrganisersTab backendOnline={backendOnline} />
              )}
            </div>
          )}
        </div>
          ...
          >
            ...
            >
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: "#9ca3af" }}>
                  Founders (A-0001, A-0002, A-0003) are protected and cannot be
                  removed.
                </p>
              </div>
              <button
            ...
            {showAddAdminForm ? (
              ...
              >
                ...
                <p className="text-xs" style={{ color: "#9ca3af" }}>
                  Password will be SHA-256 hashed by the backend.
                </p>
              ...
              <div className="p-6 flex-1 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  ...
                  <button
                    ...
                    style={{
                      background:
                        mailMode === "particular" ? "#1a1a1a" : "#f0f0f0",
              ...
              <>
                {mailSent ? (
                  ...
                  <>
                    ...
                    >
                      <p className="text-xs" style={{ color: "#9ca3af" }}>
                        From: eventsphere.noreply@gmail.com
                      </p>
                      <button
