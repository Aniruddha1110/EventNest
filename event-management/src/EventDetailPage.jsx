...
import { Calendar, Clock, MapPin, Users, Tag, ChevronLeft, Check, Star, AlertCircle } from "lucide-react";
  ...
  <div className="flex gap-1">
    {[1,2,3,4,5].map(star => (
      <button key={star} onClick={() => onChange(star)} type="button"
        ...
        <Star size={24} className={star <= value ? "fill-[#C4F249] text-[#C4F249]" : "text-gray-300"} />
      </button>
    ))}
  </div>
);
  ...
  <div className="bg-white border border-gray-100 p-4">
    <div className="flex items-start justify-between gap-2 mb-2">
      ...
      <div className="flex gap-0.5">
        {[1,2,3,4,5].map(s => (
          <Star key={s} size={14} className={s <= fb.rating ? "fill-[#C4F249] text-[#C4F249]" : "text-gray-200"} />
        ))}
      </div>
    </div>
...
const ProgrammeCard = ({ prog, selected, onToggle, isFree, disabled }) => (
  <div onClick={() => !disabled && prog.seatsLeft > 0 && onToggle(prog.id)}
    className={`border-2 p-5 transition-all ${
      disabled         ? "border-gray-100 opacity-60 cursor-default" :
      prog.seatsLeft===0 ? "border-gray-100 opacity-50 cursor-not-allowed" :
      selected         ? "border-black cursor-pointer" :
    ...
    <div className="flex items-start justify-between gap-3 mb-3">
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          {!disabled && (
            <div className={`w-5 h-5 border-2 flex items-center justify-center shrink-0 transition ${selected ? "bg-black border-black" : "border-gray-300"}`}>
              {selected && <Check size={12} className="text-[#C4F249]" />}
            </div>
          )}
          <h4 className="font-bold text-black">{prog.name}</h4>
          {prog.seatsLeft === 0 && <span className="text-xs font-bold bg-red-100 text-red-600 px-2 py-0.5">FULL</span>}
        </div>
        <p className="text-sm text-gray-500 leading-relaxed ml-7">{prog.description}</p>
      </div>
    ...
    <div className="ml-7 space-y-2">
      <div className="flex items-center gap-4 text-xs text-gray-400">
        <span className="flex items-center gap-1"><MapPin size={12} />{prog.venueName}</span>
        <span className="flex items-center gap-1"><Users size={12} />Cap. {prog.venueCapacity.toLocaleString()}</span>
      </div>
      {!disabled && <SeatBar left={prog.seatsLeft} capacity={prog.venueCapacity} />}
    </div>
  </div>
);
...
const EventDetailPage = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const mock = getEventById(eventId) || MOCK_EVENTS.find(e => e.id === eventId);
        setEvent(mock || null);
  ...
  const handleSubmitFeedback = () => {
    ...
    const newFb = { user: "You", rating: fbRating, comment: fbComment, date: new Date().toISOString().split("T")[0] };
  ...
  const handleProceed = () => {
    ...
    if (isFree) {
      navigate(`/events/${eventId}/ticket`, { state: { event, selectedProgrammes: selProgs, totalPrice: 0, isFree: true } });
    } else {
      navigate(`/events/${eventId}/payment`, { state: { event, selectedProgrammes: selProgs, totalPrice: total } });
    }
  };

  if (loading) return (
    <div className="min-h-screen bg-[#F9F9F9] flex items-center justify-center font-sans">
      <div className="animate-pulse text-xl font-bold text-gray-300">Loading event...</div>
    </div>
  );
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            {isCompleted && (
              <div className="bg-gray-100 text-gray-600 px-5 py-3 flex items-center gap-3 text-sm font-medium">
                ✓ This event has ended. You can view details and leave feedback below.
              </div>
            )}
            ...
            <div>
              <div className="flex flex-wrap gap-2 mb-4">
                <span className={`text-xs font-bold px-2.5 py-1 uppercase tracking-wide ${
                  isOngoing ? "bg-black text-[#C4F249]" : isUpcoming ? "bg-[#C4F249] text-black" : "bg-gray-200 text-gray-600"}`}>
                  {event.status}
                </span>
                <span className={`text-xs font-bold px-2.5 py-1 uppercase ${isFree ? "bg-[#C4F249] text-black" : "bg-black text-[#C4F249]"}`}>
                  {event.type}
                </span>
                {avgRating && <span className="text-xs font-bold px-2.5 py-1 bg-white border border-gray-200">⭐ {avgRating} ({feedbacks.length} reviews)</span>}
              </div>
              <h1 className="text-[#C4F249] text-5xl md:text-6xl font-normal leading-[1.1] tracking-tight mb-4">{event.name}</h1>
              <p className="text-gray-600 text-base leading-relaxed max-w-2xl">{event.description}</p>
            </div>
            ...
            <div className="bg-white border border-gray-100 p-5">
              <p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">Organised by</p>
              <p className="text-xl font-bold text-black">{event.organiserName}</p>
            </div>
            ...
            <div>
              ...
              {(event.programmes || []).length === 0 ? (
                <div className="bg-white border border-dashed border-gray-200 p-10 text-center">
                  <p className="text-gray-400 text-sm">Programme details will be available soon.</p>
                </div>
              ) : (
              ...
              <div>
                ...
                {!fbSent ? (
                  <div className="bg-white border border-gray-100 p-6 mb-6">
                    <h3 className="font-bold text-black mb-4">Leave Your Feedback</h3>
                    <div className="space-y-4">
                      <div>
                        <p className="text-xs text-gray-400 font-semibold uppercase mb-2">Your Rating</p>
                        <StarRating value={fbRating} onChange={setFbRating} />
                      </div>
                      <div>
                        <p className="text-xs text-gray-400 font-semibold uppercase mb-2">Your Comment</p>
                        <textarea
                          rows={4}
                  ...
                  <div className="bg-[#C4F249] border border-black p-5 mb-6 flex items-center gap-3">
                    ...
                    <p className="font-bold text-black">Thank you! Your feedback has been submitted.</p>
                  </div>
                )}
                ...
                {feedbacks.length > 0 ? (
                  <div className="space-y-3">
                    {feedbacks.map((fb, i) => <FeedbackCard key={i} fb={fb} />)}
                  </div>
                ) : (
            ...
            <div className="bg-white border border-gray-100 p-6">
              ...
              {isOngoing && (
                <div className="space-y-3 mb-4">
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed">
                    This event is in progress. You cannot purchase tickets for ongoing events.
                    Visit the venue directly to check availability.
                  </p>
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between"><span className="text-gray-400">Organiser</span><span className="font-bold text-black">{event.organiserName}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">Programmes</span><span className="font-bold text-black">{event.programmes?.length || 0}</span></div>
                  </div>
                </div>
              )}
                ...
                <>
                  {selectedProgrammes.length === 0 ? (
                    <div className="text-center py-8 border border-dashed border-gray-200 mb-4">
                      <p className="text-sm text-gray-400">No programmes selected</p>
                      <p className="text-xs text-gray-300 mt-1">Select from the list on the left</p>
                    </div>
                  ) : (
                    <div className="space-y-2 mb-4">
                      {selectedProgrammes.map(p => (
                        <div key={p.id} className="flex justify-between items-center text-sm py-2 border-b border-gray-50">
                          ...
                          <span className="font-bold text-black">{isFree ? "Free" : `₹${p.price}`}</span>
                        </div>
                      ))}
                      {!isFree && (
              ...
              {isCompleted && (
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between"><span className="text-gray-400">Organiser</span><span className="font-bold text-black">{event.organiserName}</span></div>
                  <div className="flex justify-between"><span className="text-gray-400">Programmes</span><span className="font-bold text-black">{event.programmes?.length || 0}</span></div>
                  <div className="flex justify-between"><span className="text-gray-400">Total Reviews</span><span className="font-bold text-black">{feedbacks.length}</span></div>
                  {avgRating && <div className="flex justify-between"><span className="text-gray-400">Avg. Rating</span><span className="font-bold text-black">⭐ {avgRating} / 5</span></div>}
                  <div className="bg-gray-100 text-gray-600 text-center py-3 text-xs font-bold uppercase tracking-wide">Event Completed</div>
                </div>
              )}
            </div>
            <p className="text-xs text-gray-300 text-center font-mono">{event.id}</p>
          </div>
