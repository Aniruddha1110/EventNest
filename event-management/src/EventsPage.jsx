...
//           className="bg-black text-white text-sm font-bold px-3 py-1.5 tracking-wide hover:opacity-90 transition"
//         >
//           EventSphere
//         </button>
...
//           <button
//             onClick={() => navigate("/")}
//             className="bg-black text-white text-sm font-semibold px-4 py-1.5 hover:opacity-80 transition"
//           >
//             Logout
//           </button>
//         </div>
    ...
    <div onClick={onClick}
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-3 mb-3">
          <h3 className="text-xl font-bold text-black group-hover:opacity-70 transition leading-tight">{event.name}</h3>
          <div className="flex flex-col items-end gap-1 shrink-0">
            <span className={`text-xs font-bold px-2.5 py-1 uppercase tracking-wide ${statusStyle.badge}`}>{event.status}</span>
            <span className={`text-xs font-bold px-2 py-0.5 ${event.type === "Paid" ? "bg-black text-[#C4F249]" : "bg-[#C4F249] text-black"}`}>{event.type}</span>
          </div>
        </div>
        <p className="text-sm text-gray-500 leading-relaxed mb-4 line-clamp-2 flex-1">{event.description}</p>
        <div className="flex flex-wrap gap-3 text-xs text-gray-400 mb-4">
          <span className="flex items-center gap-1"><Calendar size={12} />{new Date(event.startDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
        ...
        <div className="pt-3 border-t border-gray-100 flex justify-between items-center">
          ...
          <span className="text-xs font-bold text-black group-hover:opacity-60 transition">{ctaLabel}</span>
        </div>
      </div>
    </div>
  );
  ...
  <section className="mb-16">
    ...
    {events.length === 0 ? (
      <div className="bg-white border border-dashed border-gray-200 py-12 text-center">
        <p className="text-sm text-gray-400">No {title.toLowerCase()} right now</p>
      </div>
    ) : (
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {events.slice(0, 3).map(e => <EventCard key={e.id} event={e} onClick={() => onCardClick(e.id)} />)}
      </div>
    )}
  </section>
);
...
const EventsPage = () => {
  ...
  const upcoming  = filtered.filter(e => e.status === "upcoming" && new Date(e.startDate) > today);
    ...
    <div className="min-h-screen bg-[#F9F9F9] font-sans selection:bg-lime-200">
      ...
      <nav className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
        ...
        <div className="flex items-center gap-5">
          <button onClick={() => navigate("/events/ongoing")}  className="text-sm font-medium text-gray-500 hover:text-black transition">Ongoing</button>
          <button onClick={() => navigate("/events/upcoming")} className="text-sm font-medium text-gray-500 hover:text-black transition">Upcoming</button>
          <button onClick={() => navigate("/userprofile")}     className="text-sm font-medium text-gray-500 hover:text-black transition">My Profile</button>
          <button onClick={() => navigate("/")} className="bg-black text-white text-sm font-semibold px-4 py-1.5 hover:opacity-80 transition">Logout</button>
        </div>
      </nav>

      {/* Hero */}
      <div className="max-w-7xl mx-auto px-6 pt-8 pb-10">
        <h1 className="text-[#C4F249] text-6xl md:text-7xl font-normal leading-[1.1] tracking-tight mb-4">Events</h1>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <p className="text-gray-400 text-sm">
            {ongoing.length} ongoing · {upcoming.length} upcoming · {completed.length} completed
            {!backendOnline && <span className="ml-3 text-yellow-600 text-xs">⚠ Mock data — backend offline</span>}
          </p>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20">
        <Section title="Ongoing Events"   subtitle="Happening right now"             events={ongoing}   accentColor="#C4F249" onCardClick={id => navigate(`/events/${id}`)} viewAllPath="/events/ongoing"   navigate={navigate} />
        <Section title="Upcoming Events"  subtitle="Register early — limited seats"  events={upcoming}  accentColor="#1a1a1a" onCardClick={id => navigate(`/events/${id}`)} viewAllPath="/events/upcoming"  navigate={navigate} />
        <Section title="Completed Events" subtitle="Past events — leave your feedback" events={completed} accentColor="#9ca3af" onCardClick={id => navigate(`/events/${id}`)} viewAllPath="/events/completed" navigate={navigate} />
      </div>
    </div>
  );
};
